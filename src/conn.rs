...
impl<'a, 'c> InProgress<'a, 'c> {
    ...
    pub fn last_tx_id(&self) -> Entid {
        *self.partition_map[":db.part/tx"].get_index() - 1
    }
}
...
impl Conn {
    ...
    pub fn last_tx_id(&self) -> Entid {
        ...
        *metadata.partition_map[":db.part/tx"].get_index() - 1
    }
...
mod tests {
    ...
    fn test_transact_does_not_collide_existing_entids() {
        ...
        let next = *conn.metadata.lock().expect("metadata")
                       .partition_map[":db.part/user"].get_index();
    ...
    fn test_transact_does_not_collide_new_entids() {
        ...
        // Let's find out the next ID that'll be allocated. We're going to try to collide with it.
        let next = *conn.metadata.lock().expect("metadata").partition_map[":db.part/user"].get_index();
    ...
    fn get_next_entid(conn: &Conn) -> i64 {
        ...
        *partition_map.get(":db.part/user").unwrap().get_index()
    }
