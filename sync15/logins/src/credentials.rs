...
pub fn get_credential<Q>(queryable: &Q, id: CredentialId) -> Result<Option<Credential>> where Q: Queryable {
    ...
    let credential = match scalar {
        Some(Binding::Map(cm)) => {
            let username = cm.get(&*CREDENTIAL_USERNAME).and_then(|username| username.as_string()).map(|x| (**x).clone()); // XXX
            let password = cm[CREDENTIAL_PASSWORD.clone()].as_string().map(|x| (**x).clone()).unwrap(); // XXX
            let created_at = cm[CREDENTIAL_CREATED_AT.clone()].as_instant().map(|x| (*x).clone()).unwrap(); // XXX
            let title = cm.get(&*CREDENTIAL_TITLE).and_then(|username| username.as_string()).map(|x| (**x).clone()); // XXX
            // TODO: device.
