...
impl Generation {
    ...
    fn temp_id_avs<'a>(&'a self) -> Vec<(TempId, AVPair)> {
...
mod tests {
    ...
    fn assert_transactions(conn: &rusqlite::Connection, db: &mut DB, transactions: &Vec<edn::Value>) {
        for (index, transaction) in transactions.into_iter().enumerate() {
            ...
            let expected_error_message: Option<&edn::Value> = transaction.get(&edn::Value::NamespacedKeyword(symbols::NamespacedKeyword::new("test", "expectederrormessage")));
